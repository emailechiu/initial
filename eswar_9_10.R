call_sensitivity<-read.csv('eswar_9_10.csv')
call_sensitivity$prob<-call_sensitivity$Nom/call_sensitivity$Denom #split(call_sensitivity,Category)
...
call_sensitivity$Cat <- factor(call_sensitivity$Category,levels=c("Billing","Communication from HNS","Cost of Service","Customer Service", "Service Performance","Service Reliability","Retention","Retention Performance", "Retention Reliability", "Others"),labels=c("Bil","Com","Cost","Cust","Perf","Rel","Ret", "Ret-Perf", "Ret-Rel","Oth")) 
library(ggplot2);library(scales)
pline<-ggplot(call_sensitivity)+aes(Track,prob,group=Category,colour=Cat)+geom_line()+facet_grid(Month~.)+geom_text(aes(label=Cat),size = 3)+scale_y_continuous(limits=c(0,0.16),labels=percent)#+scale_colour_brewer(palette="Set1")
pbar<-ggplot(call_sensitivity)+aes(Cat,fill=Cat)+geom_bar(aes(y=prob),stat="identity")+facet_grid(Month~Track)+scale_y_continuous(limits=c(0,0.16),labels=percent)
...
ggplot(call_sensitivity)+ aes(Track,prob) +geom_point(colour=as.numeric(factor(Cat))) +scale_colour_brewer(pal = "Set1" )
